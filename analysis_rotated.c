...
int main()
{
    ...
    sprintf(file_name_dip, "results-analysis/dipole.dat");
    out_dip=fopen(file_name_dip,"w");

    sprintf(file_name_quad, "results-analysis/cell_quadrdupole.dat");
    ...
    sprintf(file_name_monop, "results-analysis/monopole.dat");
...
// begins the repetitions
  for(k=N_RIP0; k<N_RIP; k++){
      ...
      /***** media, varianza, max e min  ******/
        ...
        fprintf(out_monop,"%d\t %lf\t %lf\n",k, meanvelx, variancevelx);
